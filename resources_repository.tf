module "repository" {
  ...
  visibility                  = try(each.value.github_repository.visibility, "internal")
  ...
  squash_merge_commit_title   = try(each.value.github_repository.squash_merge_commit_title, "PR_TITLE")
  squash_merge_commit_message = try(each.value.github_repository.squash_merge_commit_message, "PR_BODY")
  merge_commit_title          = try(each.value.github_repository.merge_commit_title, "PR_TITLE")
  merge_commit_message        = try(each.value.github_repository.merge_commit_message, "PR_BODY")
